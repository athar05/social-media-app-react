...
const SignupForm = () => {
  ...
  const submitHandler = (event) => {
    ...
    if (enteredFirstName.length <= 1) {
      dispatch(
        setAlert("First Name Should Be More Than 1 Character", "error", id)
      );
      ...
      !username_condition_1.test(enteredUsername) ||
      enteredUsername.length < 4
    ) {
      dispatch(
        setAlert(
          "Username Should Have Atleast 4 Characters And Can Only Contain Letters & Numbers",
          "error",
          id
        )
      );
      ...
      dispatch(
        setAlert(
          "Password's Length Should Be More Than 6 Characters",
          "error",
          id
        )
      );
      ...
      dispatch(
        setAlert("Password Must Contain Atleast One Number", "error", id)
      );
      setTimeout(() => dispatch(removeAlert(id)), 5000);
    } else if (enteredPassword !== reEnteredPassword) {
      ...
      dispatch(
        setAlert(
          "Passwords Must Contain At Least One Smallcase Letter",
          "error",
          id
        )
      );
